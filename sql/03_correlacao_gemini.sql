...
-- --------------------------------------------------------------------
CREATE OR REPLACE TABLE `cs-demo-2026.cs_frotas_data.tb_vetor_descricoes_unicas` AS
SELECT
...
FROM `cs-demo-2026.cs_frotas_data.relatorio_item_vetor`
...
-- --------------------------------------------------------------------
CREATE OR REPLACE TABLE `cs-demo-2026.cs_frotas_data.tb_sap_materiais_unicos` AS
SELECT
  TRIM(material) AS codigo_sap,
  TRIM(UPPER(`texto_breve_de_material`)) AS descricao_sap_limpa,
  COUNT(*) AS total_registros_estoque
FROM `cs-demo-2026.cs_frotas_data.relatorio_estoque_sap_mb52`
...
-- --------------------------------------------------------------------
CREATE OR REPLACE TABLE `cs-demo-2026.cs_frotas_data.tb_depara_vetor_sap_gemini` AS
SELECT
...
FROM
  ML.GENERATE_TEXT(
    ...
    (
      SELECT
        CONCAT(
          ...
          (
            ...
            FROM `cs-demo-2026.cs_frotas_data.tb_sap_materiais_unicos`
            LIMIT 50
          ),
      ...
      FROM `cs-demo-2026.cs_frotas_data.tb_vetor_descricoes_unicas`
    ),
    STRUCT(
      0.1 AS temperature,
...
-- --------------------------------------------------------------------
CREATE OR REPLACE TABLE `cs-demo-2026.cs_frotas_data.tb_cruzamento_vetor_sap_consolidado` AS
SELECT
  v.*,
...
LEFT JOIN `cs-demo-2026.cs_frotas_data.tb_vetor_descricoes_unicas` u
  ON FARM_FINGERPRINT(TRIM(UPPER(v.`descrição`))) = u.hash_descricao_vetor
LEFT JOIN `cs-demo-2026.cs_frotas_data.tb_depara_vetor_sap_gemini` depara
